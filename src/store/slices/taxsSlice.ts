...
import {E_TaxStatus, T_Tax, T_TaxsFilters, T_Code} from "modules/types.ts";
...
export const fetchTaxs = createAsyncThunk<T_Tax[], object, AsyncThunkConfig>(
    ...
    async function(_, thunkAPI) {
        ...
        return response.data.filter(tax => tax.owner.includes(state.taxs.filters.owner))
    }
)
...
export const acceptTax = createAsyncThunk<void, string, AsyncThunkConfig>(
    "taxs/accept_tax",
    async function(tax_id,{dispatch}) {
        await api.taxs.taxsUpdateStatusAdminUpdate(tax_id, {status: E_TaxStatus.Completed})
        await dispatch(fetchTaxs)
    }
)

export const rejectTax = createAsyncThunk<void, string, AsyncThunkConfig>(
    "taxs/accept_tax",
    async function(tax_id,{dispatch}) {
        await api.taxs.taxsUpdateStatusAdminUpdate(tax_id, {status: E_TaxStatus.Rejected})
        await dispatch(fetchTaxs)
    }
)
